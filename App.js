...
export default function App() {
  ...
  useEffect(() => {
    ...
    if(Platform.OS === 'ios') {
      beaconsDidRange = Beacons.BeaconsEventEmitter.addListener(
        'beaconsDidRange',
        (data) => {
      ...
      beaconsDidRange = DeviceEventEmitter.addListener(
        'beaconsDidRange',
        (data) => {
          setDevice(data.beacons[0])
        }
    ...
    <View style={styles.container}>
      <View 
        style={{
            ...
            borderRadius: '50%', 
            backgroundColor: 
              device.proximity === 'immediate' || device.proximity === 'near' 
                ? 'green' 
      ...
      <View style={styles.row}>
        ...
        <Text style={styles.text}>dx_fe's mac</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.title}>Accuracy (정확도)</Text>
        <Text style={styles.text}>{device.accuracy}</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.title}>Proximity (거리)</Text>
        <Text style={styles.text}>{device.proximity}</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.title}>UUID (고유아이디)</Text>
        <Text style={styles.text}>{device.uuid}</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.title}>rssi (수신신호강도)</Text>
        <Text style={styles.text}>{device.rssi}</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.title}>minor</Text>
        <Text style={styles.text}>{device.minor}</Text>
      </View>
      <View style={styles.row}>
        <Text style={styles.title}>major</Text>
        <Text style={styles.text}>{device.major}</Text>
      </View>
    </View>
  )
}
